...
function initTatha() {
    ...
    function inithorizontalprojects() {
        if ($("#projects_horizontal_container").length) {
            var d = $("#projects_horizontal_container");
    ...
    if ($(".vertical-projects_wrap ").length > 0) {
        if ($(window).width() < 1068) {
...
// for slider move 
setInterval(() => {
    const btn = document.querySelector('.fwc_btn_next');
    if (btn) {
      btn.click(); 
    }
  }, 3000);
